...
let snap=await evaluate(`(()=>{const s=qaStateV84();window.state=s;applyZeroResourceOpeningV112(s);delete s.knowledgeV123;ensureKnowledgeV123(s);s.explorationV113={current:'base',discovered:['base']};s.explorationV118={selected:null,explored:['base'],observed:{base:true}};renderMap();return {panel:!!document.querySelector('.knowledge-v125'),text:document.getElementById('map')?.innerText||''};})()`);await sleep(30);
snap=await evaluate(`(()=>({panel:!!document.querySelector('.knowledge-v125'),text:document.getElementById('map')?.innerText||''}))()`);
...
await evaluate(`learnIntelV123('I-R01',{source:'observation',verified:true});renderMap()`);await sleep(30);
...
await evaluate(`learnIntelV123('I-C01',{source:'document'});evaluateDeductionsV123();renderMap()`);await sleep(30);
...
await evaluate(`learnIntelV123('I-C08',{source:'observation',verified:true});evaluateDeductionsV123();renderMap()`);await sleep(30);

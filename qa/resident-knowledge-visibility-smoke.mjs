...
await evaluate(`(()=>{document.querySelectorAll('.knowledge-v125,.documents-v124').forEach(el=>el.remove());const s=qaStateV84();window.state=s;applyZeroResourceOpeningV112(s);delete s.coreNpcV121;delete s.socialV122;delete s.knowledgeV123;delete s.discoveryV124;delete s.sceneInteractionsV119;ensureCoreNpcStateV121(s);ensureSocialV122(s);ensureKnowledgeV123(s);ensureDiscoveryV124(s);ensureSceneStateV119(s);s.explorationV113={current:'base',discovered:['base']};s.explorationV118={selected:null,explored:['base'],observed:{base:true}};return true})()`);
await sleep(80);
let snap=await evaluate(`(()=>({panel:!!document.querySelector('.knowledge-v125'),text:document.getElementById('map')?.innerText||'',intel:Object.keys(state.knowledgeV123?.intelligence||{}).length}))()`);
...
await evaluate(`learnIntelV123('I-R01',{source:'observation',verified:true});renderMap()`);await sleep(80);
...
await evaluate(`learnIntelV123('I-C01',{source:'document'});evaluateDeductionsV123();renderMap()`);await sleep(80);
...
await evaluate(`learnIntelV123('I-C08',{source:'observation',verified:true});evaluateDeductionsV123();renderMap()`);await sleep(80);

...
await waitFor(()=>evaluate(`!!document.querySelector('.community-request-v128')`),3000,25);
...
assert(snap.known&&snap.visible&&snap.panel,`known local request did not appear: ${JSON.stringify(snap)}`);
...
await evaluate(`state.explorationV113.current='homes';state.backpack.items.push(physicalItemV119('item-crackers'));recalcPhysicalWeightsV115(state);renderMap()`);
await waitFor(()=>evaluate(`!!document.querySelector('.community-request-v128')`),3000,25);

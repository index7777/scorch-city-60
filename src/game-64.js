...
function refillDiagnosticBatteryV64(id){
 const t=ensureFieldTeamV43(),r=diagnosticRuntimeV64(id),d=DIAGNOSTIC_RUNTIME_DEFS_V64[id];if(!r||!d)return toast('尚未擁有這件量測儀表');if(t.active)return toast('外勤進行中，不能更換儀表電池');const room=Math.max(0,d.capacityKWh-r.batteryKWh);if(room<.002)return toast('儀表電池已充足');const move=Math.min(room,d.cellTopupKWh,state.resources?.battery||0);if(move<=.001)return toast('中央庫存沒有可用電池電量');state.resources.battery=Math.max(0,(state.resources.battery||0)-move);r.batteryKWh=Math.min(d.capacityKWh,r.batteryKWh+move);log(`${d.label}更換／補充儀表電池 ${move.toFixed(2)} kWh。`,'good');openInventory();saveGame(false)
}
...
function calibrateDiagnosticToolV64(id){
 const r=diagnosticRuntimeV64(id),d=DIAGNOSTIC_RUNTIME_DEFS_V64[id],site=calibrationSiteV64();if(!r||!d)return toast('尚未擁有這件量測儀表');if(!site)return toast('必須回到耐熱屋或中央通風站的已知參考點才能校準');const chen=chenMaintenanceAvailableV62(site),hours=Math.round(d.calHours*(chen?.65:1)*20)/20;if(chen&&npcDutyRemainingV41('chen')+1e-6<hours)return toast('陳技師今日剩餘工時不足');if(!spendWorldTimeV26(hours,{label:`校準${d.label}`}))return;const before=diagnosticCalibrationV64(id);r.calibration=100;r.lastCalDay=state.day;r.condition=Math.min(100,r.condition+.8);if(chen)useNpcDutyV41('chen',hours,`量測儀表校準：${d.label}`);log(`${d.label}完成參考點校準：${before.toFixed(0)}% → 100%。`,'good');openInventory();saveGame(false)
}
...
diagnoseAssetV63=function(id,depth='basic'){
 ...
 for(const f of active){const req=FAULT_MEASUREMENTS_V63[f.id]||[],qualities=req.map(x=>toolQual[x]||0),coverage=req.length?qualities.reduce((a,b)=>a+b,0)/req.length:1,matched=qualities.filter(x=>x>0).length,old=faultEvidenceV63(id,f.id);let gain=.16+(plan.deep?.14:0)+(plan.chen?.14:0)+coverage*.38;if(req.length&&!matched)gain-=.10;const evidenceCap=req.length?(coverage<.10?(plan.chen?.76:.70):coverage<.55?(plan.chen?.84:.79):coverage<.80?.90:.97):.97,confidence=clamp(Math.min(evidenceCap,Math.max(old.confidence,0)+gain),0,.97),qualityPenalty=req.length?(1-coverage)*18:0,spread=Math.max(6,44-confidence*34+qualityPenalty),center=f.severity;fs.evidence[f.id]={confidence,severityMin:clamp(center-spread,0,100),severityMax:clamp(center+spread,0,100),runs:(old.runs||0)+1,tools:[...new Set([...(old.tools||[]),...plan.tools])],lastDay:state.day,measurementQuality:coverage};if(confidence>=.78){if(!fs.diagnosed.includes(f.id))fs.diagnosed.push(f.id);confirmed++}}
...
function diagnosticRuntimeHtmlV64(){
 ensureDiagnosticRuntimeV64();const t=ensureFieldTeamV43(),site=calibrationSiteV64(),rows=Object.entries(DIAGNOSTIC_TOOL_DEFS_V63).filter(([id])=>ensureDiagnosticToolsV63()[id]).map(([id,d])=>{const r=diagnosticRuntimeV64(id),def=DIAGNOSTIC_RUNTIME_DEFS_V64[id],bat=diagnosticBatteryPctV64(id),cal=diagnosticCalibrationV64(id),acc=diagnosticAccuracyV64(id),warn=acc<.78?'warn':'';return `<div class="diag-runtime-row ${warn}"><div><b>${d.label}</b><small>${diagnosticAccuracyLabelV64(acc)} · 精度 ${Math.round(acc*100)}% · 校準 ${cal.toFixed(0)}%</small></div><div class="diag-runtime-values"><span>電池 ${r.batteryKWh.toFixed(2)} / ${def.capacityKWh.toFixed(2)} kWh</span><span>使用 ${r.uses} 次</span></div><div class="diag-runtime-actions"><button class="mini secondary" data-diag-refill="${id}" ${t.active||bat>=98?'disabled':''}>補充電池</button><button class="mini secondary" data-diag-calibrate="${id}" ${!site?'disabled':''}>重新校準 · ${def.calHours.toFixed(2)}h</button></div></div>`}).join('')||'<p class="muted">尚未製作量測儀表。</p>';return `<section class="diag-runtime-panel"><div class="source-load-head"><div><span>INSTRUMENT QUALITY</span><h3>量測儀表狀態</h3></div></div><p class="muted">低電量、校準漂移與儀表狀況會直接降低量測品質並放大故障嚴重度區間。校準只能在耐熱屋／中央通風站的已知參考點進行；外勤現場不能免費校正。</p><div class="diag-runtime-list">${rows}</div></section>`}

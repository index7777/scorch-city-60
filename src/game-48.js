...
function fieldToolBudgetV48(e){
 ...
 if(driveReserve>0&&budget.vehicleKWh<-.001)issues.push('工程車行駛／車艙冷卻已超出目前車載電量');
 for(const stop of ensureItineraryV27().stops){const use=stopToolUseV48(stop),before={...budget};budget.toolkitKWh-=use.toolkitKWh;budget.toolkitCondition-=use.toolkitWear;budget.cartCondition-=use.cartWear;budget.liftCondition-=use.liftWear;budget.vehicleCondition-=use.vehicleWear;budget.vehicleKWh-=use.liftKWh;budget.medicine-=use.medicine;const name=mapLoc(stop.location)?.name||stop.location;
  const usesToolkit=use.toolkitKWh>1e-6||use.toolkitWear>1e-6,usesCart=use.cartWear>1e-6,usesLift=use.liftKWh>1e-6||use.liftWear>1e-6,usesVehicle=use.vehicleWear>1e-6||driveReserve>0,usesMedicine=use.medicine>1e-6;
  if(usesToolkit&&budget.toolkitKWh<-.001)issues.push(`${name}：遠征工具電池不足`);if(usesToolkit&&budget.toolkitCondition<10)issues.push(`${name}：遠征工具箱耐久不足`);if(usesCart&&budget.cartCondition<10)issues.push(`${name}：推車狀況不足`);if(usesLift&&budget.liftCondition<10)issues.push(`${name}：液壓搬運平台狀況不足`);if(usesVehicle&&budget.vehicleCondition<15)issues.push(`${name}：工程車狀況不足`);if((driveReserve>0||usesLift)&&budget.vehicleKWh<-.001)issues.push(`${name}：車載電池不足以同時支撐行駛與液壓平台`);if(usesMedicine&&budget.medicine<-.001)issues.push(`${name}：醫療耗材不足`);rows.push({stop,use,before,after:{...budget}})
 }

...
function toolConditionV48(id){
 if(id==='vehicle')return clamp(ensureVehicleStateV32().condition??100,0,100);
 if(id==='lift'||id==='toolkit'||id==='cart')return clamp(fieldToolRuntimeV48(id)?.condition??100,0,100);
 return 100
}
...
function fieldToolBudgetV48(){
 const r=ensureFieldToolRuntimeV48(),cv=coolingVehicleV32(),budget={toolkitKWh:r.toolkit?.battery?.chargeKWh||0,toolkitCondition:r.toolkit?.condition??0,cartCondition:r.cart?.condition??0,liftCondition:r.lift?.condition??0,vehicleCondition:state.gear?.vehicle?(ensureVehicleStateV32().condition??0):0,vehicleKWh:cv?.battery?.chargeKWh||0,medicine:state.resources?.medicine||0},rows=[],issues=[];
 for(const stop of ensureItineraryV27().stops){const use=stopToolUseV48(stop),before={...budget};budget.toolkitKWh-=use.toolkitKWh;budget.toolkitCondition-=use.toolkitWear;budget.cartCondition-=use.cartWear;budget.liftCondition-=use.liftWear;budget.vehicleCondition-=use.vehicleWear;budget.vehicleKWh-=use.liftKWh;budget.medicine-=use.medicine;const name=mapLoc(stop.location)?.name||stop.location;
  if(budget.toolkitKWh<-.001)issues.push(`${name}：遠征工具電池不足`);if(budget.toolkitCondition<10)issues.push(`${name}：遠征工具箱耐久不足`);if(budget.cartCondition<10)issues.push(`${name}：推車狀況不足`);if(budget.liftCondition<10)issues.push(`${name}：液壓搬運平台狀況不足`);if(budget.vehicleCondition<15)issues.push(`${name}：工程車狀況不足`);if(budget.vehicleKWh<-.001)issues.push(`${name}：車載電池不足以供液壓平台工作`);if(budget.medicine<-.001)issues.push(`${name}：醫療耗材不足`);rows.push({stop,use,before,after:{...budget}})
 }
 return {start:{toolkitKWh:r.toolkit?.battery?.chargeKWh||0,toolkitCondition:r.toolkit?.condition??0,cartCondition:r.cart?.condition??0,liftCondition:r.lift?.condition??0,vehicleCondition:state.gear?.vehicle?(ensureVehicleStateV32().condition??0):0,vehicleKWh:cv?.battery?.chargeKWh||0,medicine:state.resources?.medicine||0},end:budget,rows,issues:[...new Set(issues)],ok:issues.length===0}
}
...
/* Current-stop planning checks instantaneous tool health; itinerary validation additionally checks cumulative budget. */
const _fieldTeamValidationV48=fieldTeamValidationV43;
fieldTeamValidationV43=function(e){const v=_fieldTeamValidationV48(e),b=fieldToolBudgetV48(),issues=[...v.issues,...b.issues];return {...v,toolBudget:b,ok:issues.length===0,issues:[...new Set(issues)]}};
...
function consumeStopToolUseV48(stop){
 ...
 if(r.toolkit){r.toolkit.condition=clamp(r.toolkit.condition-u.toolkitWear,0,100);r.toolkit.battery.chargeKWh=Math.max(0,r.toolkit.battery.chargeKWh-u.toolkitKWh)}
 if(r.cart)r.cart.condition=clamp(r.cart.condition-u.cartWear,0,100);
 if(r.lift)r.lift.condition=clamp(r.lift.condition-u.liftWear,0,100);
...
runItineraryStepV27=function(){const it=ensureItineraryV27(),before=it.index,stop=it.stops[before];const out=_runItineraryStepV48();if(stop&&it.index>before)consumeStopToolUseV48(stop);return out};
...
function fieldToolRuntimeHtmlV48(){
 ...
 if(r.toolkit)rows.push(`<div class="tool-runtime-row"><div><b>遠征工具箱</b><small>耐久 ${r.toolkit.condition.toFixed(0)}%</small></div><span>工具電池 ${r.toolkit.battery.chargeKWh.toFixed(2)} / ${r.toolkit.battery.capacityKWh.toFixed(2)} kWh</span></div>`);
 if(r.cart)rows.push(`<div class="tool-runtime-row"><div><b>推車</b><small>耐久 ${r.cart.condition.toFixed(0)}%</small></div><span>機械式搬運</span></div>`);
 if(r.lift)rows.push(`<div class="tool-runtime-row"><div><b>液壓搬運平台</b><small>耐久 ${r.lift.condition.toFixed(0)}%</small></div><span>車載電 ${cv?(cv.battery.chargeKWh||0).toFixed(2)+' kWh':'不可用'}</span></div>`);
 if(state.gear?.vehicle)rows.push(`<div class="tool-runtime-row"><div><b>工程車</b><small>車況 ${ensureVehicleStateV32().condition.toFixed(0)}%</small></div><span>重型作業會額外耗損</span></div>`);
 const issues=b.issues.length?`<div class="field-team-issues">${b.issues.map(x=>`<p>${x}</p>`).join('')}</div>`:'<div class="action-ready">目前整段行程的工具耐久／電量預算可成立。</div>';
 return `<div class="tool-runtime-box"><div class="field-role-head"><div><span>TOOL RUNTIME</span><h4>工具壽命與能源</h4></div><b>${b.ok?'預算成立':'資源不足'}</b></div><div class="tool-runtime-list">${rows.join('')||'<p class="muted">目前沒有可追蹤的外勤工具。</p>'}</div>${issues}<div class="dialog-actions"><button id="reloadToolBattery" class="secondary">補充工具電池</button><button id="serviceFieldTools" class="secondary">保養外勤工具</button></div></div>`
}

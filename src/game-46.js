...
function fieldToolInventoryV46(){
 ...
 if(state.installed?.lift&&t.useVehicle&&state.gear?.vehicle&&ensureVehicleStateV32().currentLocation===home)out.push({id:'lift',...FIELD_TOOL_DEFS_V46.lift,location:home});
...
function subtaskToolRuleV46(stop,task){
 ...
 if(action==='asset'&&task.id==='load'){
  if(w>=500&&asset?.id!=='lift')return {required:['vehicle','lift'],compatible:['vehicle','lift'],reason:`${asset?.name||'重型設備'}約 ${w}kg，需要工程車與液壓搬運平台`};
...
function subtaskToolStatusV46(stop,task){
 const rule=subtaskToolRuleV46(stop,task),assign=ensureStopToolAssignmentsV46(stop),available=new Set(fieldToolInventoryV46().map(x=>x.id)),selected=assign[task.id]||null;
 const missing=rule.required.filter(id=>!available.has(id));
 const wrong=selected&&!available.has(selected)?[selected]:[];
 return {rule,selected,tool:selected?fieldToolByIdV46(selected):null,missing:[...missing,...wrong],ok:missing.length===0&&wrong.length===0}
}
...
function toolThroughputV46(tool){return tool?.throughput||1}

/* Replace V45's person-only parallel plan with a small job-shop schedule: person and unique tool are both exclusive resources. */
stopParallelPlanV45=function(stop){
 const base=rawStopActionHoursV45(stop),defs=stopSubtaskDefsV45(stop),assign=ensureStopSubtasksV45(stop),tools=ensureStopToolAssignmentsV46(stop),members=fieldTeamMembersV43(),memberReady={},toolReady={},issues=[];
 for(const id of members)memberReady[id]=0;
 const tasks=[];
 for(const d of defs){
  const member=members.includes(assign[d.id])?assign[d.id]:'player',ts=subtaskToolStatusV46(stop,d),tool=ts.tool,tp=memberThroughputV45(member,d)*toolThroughputV46(tool),work=base*d.weight,hours=work/Math.max(.01,tp);
  if(!ts.ok)issues.push(ts.rule.reason||`${d.label}缺少必要工具`);
  const start=Math.max(memberReady[member]||0,tool?(toolReady[tool.id]||0):0),end=start+hours;memberReady[member]=end;if(tool)toolReady[tool.id]=end;
  tasks.push({...d,member,toolId:tool?.id||null,tool,work,tp,hours,start,end,toolRule:ts.rule,toolOk:ts.ok})
 }
...
fieldTeamValidationV43=function(e){
 const v=_fieldTeamValidationV46(e),issues=[...v.issues];for(const stop of ensureItineraryV27().stops){const p=stopParallelPlanV45(stop);for(const x of p.issues)issues.push(`${mapLoc(stop.location)?.name||stop.location}：${x}`)}return {...v,ok:issues.length===0,issues:[...new Set(issues)]}
};

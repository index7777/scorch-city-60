// v14.3 Batch E/F — foreground explanations for disabled actions; rendering is driven by the central resident UI lifecycle.
(function(){
 ...
 function reasonForDisabledButtonV108(button){
  ...
  if(button.dataset?.research){const t=card?.textContent||'';if(/資料不足/.test(t))return '研究資料不足';if(/前置條件不足/.test(t))return '前置條件不足'}
  if(button.dataset?.craft){const t=card?.textContent||'';if(/材料不足/.test(t))return '施工材料不足';if(/條件未達成/.test(t))return '施工前置條件未達成'}
 ...
 function decorateDisabledActionsV108(root=document){
  ...
  for(const button of root.querySelectorAll('button:disabled')){
   ...
   if(!note||!note.classList?.contains('disabled-reason-v108')){note=document.createElement('div');note.className='disabled-reason-v108';button.insertAdjacentElement('afterend',note)}
   note.innerHTML=`<strong>目前無法執行：${reason}</strong><span>${hint}</span>`;
   button.setAttribute('aria-describedby',note.id||(note.id=`disabled-reason-v108-${++decorateDisabledActionsV108.seq}`));count++;
  }
  return count;
 }

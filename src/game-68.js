...
const PUBLIC_LOOT_V68={
 homes:['water','food'],store:['food','water'],school:['water','food'],clinic:['medicine','water'],hardware:['parts','filters'],warehouse:['water','food'],fire:['fuel','parts'],subway:['parts','battery'],industrial:['parts','coolant'],coldstore:['food','coolant'],research:['battery','medicine'],solar:['battery','parts'],vent:['parts','battery']
};
...
function fogLocationDescV68(){return '只知道這個方向存在可抵達的城市節點。先派人偵察，才能確認名稱、用途與資源標籤。'}

/* Unknown filters must not leak world truth by selectively hiding nodes. */
const _mapFilterPassV68=mapFilterPass;
mapFilterPass=function(id){if(!locationKnownV68(id))return true;return _mapFilterPassV68(id)};

/* Unknown nodes expose only position and a scout action. */
const _openLocationBaseV68=openLocation;
openLocation=function(id){
 if(locationKnownV68(id))return _openLocationBaseV68(id);
 const loc=mapLoc(id);if(!loc)return;
 $('locTitle').textContent='? 未知區域';
 $('locDesc').innerHTML=`<div class="fog-location-v68"><div class="fog-scene-v68">?</div><div><b>${directionFromBaseV68(id)}</b><p>${fogLocationDescV68()}</p></div></div>`;
...
function applySearchSpecialV68(loc,full){
 if(loc.special==='cart'&&!state.gear.cart){state.gear.cart=true;log('你找到一台可用推車，單趟載重提升到 80kg。','good')}
 if(loc.special==='vehicle'&&!state.gear.vehicle&&state.resources.fuel>=2){state.gear.vehicle=true;state.resources.fuel-=2;log('你確認並修復了一台工程車，單趟重載能力已解鎖。','good')}
 if(full&&loc.special==='coreInfo'&&!state.knownCore&&state.resources.data>=4){state.knownCore=true;log('完整搜索取得足夠熱工資料：冷源核心位置已確認。','major')}
 if(full&&loc.special==='solar'&&state.resources.data>=2)log('完整搜索取得太陽能場的逆變與配電資料。','good')
}

/* Full itinerary search keeps the existing dynamic-cargo path and is the only path to hidden loot/assets. */
const _collectStopLootV68=collectStopLootV27;
collectStopLootV27=function(loc){const out=_collectStopLootV68(loc);applySearchSpecialV68(loc,true);return out};
...
searchLocation=function(loc){
 ...
 const {gain,used}=quickSearchGainV68(loc,cargoCapacityKg());state.locations[loc.id].searched=true;state.intel[loc.id]={day:state.day,verifiedDay:state.day,summary:`快速搜索已確認「${publicLootLabelV68(loc.id)}」標籤範圍；其他區域與隱藏物資尚未完整搜索。`,source:'快速搜索',confidence:88};tutorialWaterGain(gain);applySearchSpecialV68(loc,false);
...
const _openUnknownAwareV68=openLocation;
openLocation=function(id){
 if(!locationKnownV68(id))return _openUnknownAwareV68(id);
 _openLocationBaseV68(id);const loc=mapLoc(id);

...
const REPAIR_STRATEGIES_V65={
 patch:{label:'臨時壓制',field:true,station:true,timeFactor:.58,costFactor:.55,conditionGain:.05,severityRemain:.48,recurrenceHours:18,recurrenceFactor:1.45,minConfidence:.78,desc:'最快、最省料；保留部分故障嚴重度，適合把設備撐到返站。'},
...
function repairPlanIssuesV65(plan){
 ...
 if(plan.kind==='field'&&!ensureFieldToolCarryV47().includes('toolkit'))issues.push('現場維修需要攜帶遠征工具箱');
 if(plan.kind==='field'){const load=fieldSpareLoadV50();for(const [k,n] of Object.entries(plan.spares))if((load[k]||0)<n)issues.push(`缺少${FIELD_SPARE_DEFS_V50[k]?.label||k} ×${n}`)}
...
function consumeRepairResourcesV65(plan){
 if(plan.kind==='field'){const load=fieldSpareLoadV50();for(const [k,n] of Object.entries(plan.spares))load[k]=Math.max(0,(load[k]||0)-n)}
...
function executeRepairStrategyV65(assetId,faultId,mode,kind){
 ...
 p.fault.severity=clamp(remain,0,100);p.fault.active=remain>8;
 ...
 p.fault.recurrence={mode,exposure:0,threshold:p.s.recurrenceHours,multiplier:p.s.recurrenceFactor,lastRepairDay:state.day};
 p.fault.repairHistory.push({day:state.day,mode,kind,beforeSeverity,afterSeverity:p.fault.severity,beforeCondition,afterCondition:state.assets[assetId].condition,hours:p.hours,resources:{...p.resources},spares:{...p.spares},withChen:p.withChen});
 state.assets[assetId].repairLog.push({day:state.day,type:`strategy-${mode}-${kind}`,faultId,beforeSeverity,afterSeverity:p.fault.severity,hours:p.hours,resources:{...p.resources},spares:{...p.spares}});
 if(!p.fault.active){const fs=assetFaultStateV62(assetId);fs.diagnosed=fs.diagnosed.filter(x=>x!==faultId)}
 log(`${assetDefs.find(a=>a.id===assetId)?.name||assetId}：${p.d.label}採用「${p.s.label}」；嚴重度 ${beforeSeverity.toFixed(0)}% → ${p.fault.severity.toFixed(0)}%，設備狀況 ${beforeCondition.toFixed(0)}% → ${state.assets[assetId].condition.toFixed(0)}%。`,'good');
